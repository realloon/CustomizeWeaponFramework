...
public class TraitEquippedFactors : StatPart
{
    public override void TransformValue(StatRequest req, ref float val)
    {
        ...
        {
            var factors = trait?.equippedStatOffsets?.Where(t => t?.stat == stat)?.ToList();
            if (factors.IsNullOrEmpty()) continue;
    ...
    public override string ExplanationPart(StatRequest req)
    {
        ...
        {
            var factors = t?.equippedStatOffsets?.Where(o => o.stat == stat)?.ToList();
            ...
            {
                total *= local;
                sb.AppendLine($"    Trait {t.label}: x{stat.ValueToString(local)}");
            }
        }
        if (total == 1f) return "";
